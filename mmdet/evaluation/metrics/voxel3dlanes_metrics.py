    ...
    def process_3d_lanes(self, data_batch, data_samples):
        for sample in data_samples:
            ...
            if sample.get('pred_3dlanes', None) is not None:
                ...
                # print("gt_lanes[0]", gt_lanes[0])
                # print("pred_lanes[0]", pred_lanes[0])
            ...
            elif (sample.get('pred_3dlanes', None) is None) and (sample.get('gt_lanes_vert', None) is not None):
                print(f"[WARNING]: No predicted lanes for this sample, but it has {len(sample['gt_lanes_vert'])} gt lanes")
...
def compute_lane_dist(gt_lane, pred_lane, z_samples):
    """
...
def compute_lane_errors(gt_lane, pred_lane, z_samples):
    ...
    y_err = np.abs(y_pr[valid] - y_gt[valid]) /100  # height error, converted from cms to ms
